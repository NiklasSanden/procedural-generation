...
void main() {
	...
	for (int i = 0; i < 16; i += 3) {
		vec2 texCoord = vec2(cubeIndex / 256.0, i / 16.0);
		if (texture(triTable, texCoord).x == -1) break;

		int index1A = edgeTable[int(texture(triTable, texCoord).x)][0];
		int index1B = edgeTable[int(texture(triTable, texCoord).x)][1];
		...
		texCoord.y += 1.0 / 16.0;
		int index2A = edgeTable[int(texture(triTable, texCoord).x)][0];
		int index2B = edgeTable[int(texture(triTable, texCoord).x)][1];
		...
		texCoord.y += 1.0 / 16.0;
		int index3A = edgeTable[int(texture(triTable, texCoord).x)][0];
		int index3B = edgeTable[int(texture(triTable, texCoord).x)][1];
